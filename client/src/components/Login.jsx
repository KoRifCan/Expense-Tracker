        ...
        <form onSubmit={handleLinkAccount}>
          <div className="relative mb-4">
            <input
              ...
              required
            />
            <button type="button" onMouseDown={(e) => { e.preventDefault(); const el = passwordRef.current; const pos = el?.selectionStart ?? 0; setShowPass(s => !s); requestAnimationFrame(() => { el?.focus(); el?.setSelectionRange(pos, pos); }); }} className="absolute right-3 top-3 text-gray-400 hover:text-gray-600 dark:hover:text-gray-300">
              {showPass ? (
